...
export function settleHealthDebt(state: GameState): GameState {
  ...
  const sedentaryLoad = clamp(state.healthProfile.sedentaryLoad + Math.max(0, state.hidden.fatigue - 45) * 0.05 - state.healthProfile.exerciseHabit * 0.025, 0, 100);
  const chronicRisk = clamp(state.healthProfile.chronicRisk + sleepDebt * 0.018 + sedentaryLoad * 0.018 + chronicStress * 0.02 - state.healthProfile.exerciseHabit * 0.018 - state.healthProfile.nutritionQuality * 0.01, 0, 100);
  const healthDebt = clamp(
    state.healthProfile.healthDebt + sleepDebt * 0.035 + chronicStress * 0.025 + chronicRisk * 0.02 + Math.max(0, 45 - state.healthProfile.nutritionQuality) * 0.05 - Math.max(0, recoveryBuffer) * 0.5,
    0,
    100
  );
