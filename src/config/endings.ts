...
export const CORE_ENDINGS: EndingConfig[] = [
  ...
  { id: 'burnout', title: '提前出局：长期燃尽未恢复', category: 'fail', condition: s => (s.stats.mental <= 0 || s.stats.burnout >= 100) && s.crisis.burnout.active && s.month - (s.crisis.burnout.startedMonth ?? s.month) >= 6, text: '这不是意志力不足，而是长期过载没有得到恢复窗口。系统性压力最终压过了个人硬扛。' },
  { id: 'health_fail', title: '提前出局：健康危机未恢复', category: 'fail', condition: s => s.stats.health <= 0 && s.crisis.severeIllness.active && s.month - (s.crisis.severeIllness.startedMonth ?? s.month) >= 6, text: '身体风险被拖成硬约束。真正的教训不是责备自己，而是更早把恢复和医疗支持纳入计划。' },
